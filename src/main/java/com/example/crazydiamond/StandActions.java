...
public final class StandActions {
	...
	private static final float MAX_BREAK_HARDNESS = 10.0f; // stone 1.5, obsidian 50
	...
	/** Breaks a block WITHOUT drops and logs it so Return Block can restore it. */
	static boolean smashAt(ServerWorld world, ServerPlayerEntity owner, BlockPos pos, float maxHardness) {
		...
		BrokenBlockLog.record(world, pos, state);
	...
	static void returnBlock(ServerWorld world, ServerPlayerEntity owner, StandEntity stand, int elapsed) {
		int restored = BrokenBlockLog.restoreSome(world, owner.getBlockPos(), RETURN_RADIUS, RETURN_BLOCKS_PER_TICK);
